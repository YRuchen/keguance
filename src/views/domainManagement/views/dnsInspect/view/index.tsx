...
import { getHistoryNodeListApi, getHistoryListApi, exportApi } from '~/api/domainManagement/dnsInspect'
import { Space, CommonPage, CommonTable, CommonDrawer, FormilyForm } from '~/KeepUp'
import { dnsStatusMap } from '~/api/domainManagement/common'
      ...
      <Space class={styles.container} direction='row' fill style={{ height: '100%' }}>
        ...
        <CommonDrawer
          ...
          v-slots={{
            title: () => (
              <Space>
                <ElTag type={dnsStatusMap[activeRowData.value.status]?.type}>{dnsStatusMap[activeRowData.value.status]?.label}</ElTag>
