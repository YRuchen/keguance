...
import { getHistoryNodeListApi, getHistoryListApi, exportApi } from '~/api/domainManagement/hijackDetection'
import { Space, CommonPage, CommonTable, CommonDrawer, FormilyForm } from '~/KeepUp'
import { hijackStatusMap } from '~/api/domainManagement/common'
import { getCurTime } from '~/utils/date'
import { nodeExcludeFilterKeys, recordExcludeFilterKeys } from './constants'
import { fields, getHistoryFields } from './fields'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const formatListParams = (params: ICommonObj = {}) => {
      ...
      init()
      return {
        ...Object.fromEntries(Object.entries(params).filter(([key]) => !nodeExcludeFilterKeys.includes(key))),
    ...
    const formatExportParams = (params: ICommonObj = {}) => {
      return {
        ...Object.fromEntries(Object.entries(params).filter(([key]) => !recordExcludeFilterKeys.includes(key))),
        task: route.query?.id,
    ...
    return () => (
      <Space class={styles.container} direction='row' style={{ height: '100%' }} fill>
        <CommonPage
          fields={fields}
        ...
        <CommonDrawer
          ...
          v-slots={{
            title: () => (
              <Space>
                <ElTag type={hijackStatusMap[activeRowData.value.hijackStatus]?.type}>{hijackStatusMap[activeRowData.value.hijackStatus]?.label}</ElTag>
