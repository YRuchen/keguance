...
import { getHistoryListApi, exportApi } from '~/api/domainManagement/inspectionICP'
import { Space, CommonPage, CommonDrawer, FormilyForm } from '~/KeepUp'
import { icpStatusMap } from '~/api/domainManagement/common'
import { getCurTime } from '~/utils/date'
import { getHistoryFields } from './fields'
      ...
      <Space class={styles.container} direction='row' fill style={{ height: '100%' }}>
        ...
        <CommonDrawer
          ...
          v-slots={{
            title: () => (
              <Space>
                <ElTag type={icpStatusMap[activeRowData.value.status]?.type}>{icpStatusMap[activeRowData.value.status]?.label}</ElTag>
