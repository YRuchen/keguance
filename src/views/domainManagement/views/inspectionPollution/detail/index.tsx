...
import { getHistoryNodeListApi, getHistoryListApi, exportApi } from '~/api/domainManagement/inspectionPollution'
import { polluteStatusMap } from '~/api/domainManagement/common'
    ...
    return () => (
      <Space class={styles.container} direction='row' fill style={{ height: '100%' }}>
        <CommonPage
          ...
          filterColumns={3} 
          pageKey='pollutionInspectionView'
          refreshable
          v-slots={{
        ...
        <CommonDrawer
          ...
          v-slots={{
            title: () => (
              <Space>
                <ElTag type={polluteStatusMap[activeRowData.value.polluteStatus]?.type}>{polluteStatusMap[activeRowData.value.polluteStatus]?.label}</ElTag>
