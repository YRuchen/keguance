...
import {
  ...
  disabledApi,
  getProjectsApi,
} from '~/api/domainManagement/sslInspect'
...
import { inspectStatusMap, noticeChannelOptions } from '../../common/constants'
...
export const getFields = ({ router, commonPageRef, domainMasterData }): IField[] => [
  ...
  },
  {
    ...
    columnConfig: {
      ...
      render: ({ rowData }) =>
        h(
          ElText,
          {},
          (domainMasterData?.masterData?.taskFrequencies?.sslFrequency || []).find(
    ...
    editConfig: {
      ...
      enum: domainMasterData?.masterData?.taskFrequencies?.sslFrequency,
    },
  },
  {
    prop: 'noticeMode',
