...
import { getHistoryListApi, exportApi } from '~/api/domainManagement/sslInspect'
import { sslStatusMap } from '~/api/domainManagement/common'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => (
      <Space direction='row' fill style={{ height: '100%' }}>
        <CommonPage
        ...
        <CommonDrawer
          ...
          v-slots={{
            title: () => (
              <Space>
                <ElTag type={sslStatusMap[activeRowData.value.status]?.type}>{sslStatusMap[activeRowData.value.status]?.label}</ElTag>
