...
import type { IGetListParams } from '../common/interfaces'
...
export const exportApi = async (params: ICommonObj) => {
  try {
    const res = await request.get('/api/v1/domain/domainExpiry/record/export', { params })
...
/** 检测结果列表 */
export const getListApi = async (params: IGetListParams): Promise<ICommonGetListRes<IListItem[]>> => {
  try {
...
export const createApi = async (params: ICommonObj) => {
  return request.post('/api/v1/domain/task', { ...params, taskType: TaskType.TASK_TYPE_DOMAIN })
}
...
export const getHistoryListApi = async (params: ICommonObj): Promise<ICommonGetListRes<IListItem[]>> => {
  try {
    const res: ICommonObj = await request.get('/api/v1/domain/domainExpiry/record', { params })
    return Promise.resolve({
      list: res.data.list,
      pagination: res.data.pagination,
    })
