...
export const exportApi = async (params: ICommonObj) => {
  try {
    const res = await request.get('/api/v1/domain/icp/record/export', { params })
...
export const createApi = async (params: ICommonObj) => {
  return request.post('/api/v1/domain/task', { ...params, taskType: TaskType.TASK_TYPE_ICP })
}
...
export const getHistoryListApi = async (
  ...
  try {
    const res: ICommonObj = await request.get('/api/v1/domain/icp/record', { params })
    return Promise.resolve({
      list: res.data.list,
      pagination: res.data.pagination,
    })
