...
export const exportApi = async (params: ICommonObj) => {
  try {
    const res = await request.get('/api/v1/domain/wall/record/export', { params })
...
export const createApi = async (params: ICommonObj) => {
  return request.post('/api/v1/domain/task', { ...params, taskType: TaskType.TASK_TYPE_WALL })
}
...
/** 删除 */
export const deleteApi = (id: string): Promise<Record<'status', boolean>> => {
...
export const getHistoryListApi = async (
  ...
  try {
    const res: ICommonObj = await request.get('/api/v1/domain/wall/record', { params })
    return Promise.resolve({
      list: res.data.list,
      pagination: res.data.pagination,
    })
