...
export const exportApi = async (params: ICommonObj) => {
  try {
    const res = await request.get('/api/v1/domain/dns/record/export', { params })
...
export const createApi = async (params: ICommonObj) => {
  return request.post('/api/v1/domain/task', { ...params, taskType: TaskType.TASK_TYPE_DNS })
}
...
export const getHistoryListApi = async (params: ICommonObj): Promise<ICommonGetListRes<IListItem[]>> => {
  try {
    const res: ICommonObj = await request.get('/api/v1/domain/dns/record', { params })
    return Promise.resolve({
      list: res.data.list,
      pagination: res.data.pagination,
    })
